...
void Tracking::Run() // 이거는 쓰레드로 돌아가는거임.. 따라서 계속해서 이미지 메세지를 받고, 이를 Tracking::Geabimage로 메세지를 넘겨준다.
{
    ros::NodeHandle nodeHandler;
	
	// Subscribe to the "/camera/image_raw" topic with the master. ROS will call the Tracking::GrabImage function whenever a new message arrives.
    ...
    ros::spin(); // 루프에 들어간다. 메세지 콜백을 최대한 빨리 호출한다. ros::ok()가 false가 되면 끝난다.
}

void Tracking::Run(int FIRST, int END) // 이거는 쓰레드로 돌아가는거임.. 따라서 계속해서 이미지 메세지를 받고, 이를 Tracking::Geabimage로 메세지를 넘겨준다.
{
	for (int i = FIRST; i < END; i++) {
		cv::Mat img_input = cv::imread("");
		Tracking::GrabImage(img_input);
		boost::this_thread::sleep(boost::posix_time::milliseconds(10));
	}
}

void Tracking::GrabImage(cv::Mat img_in, int i) // 이미지만 그랩하는게 아니라 tracking, relocalization, 등을 수행
{
	...
	///////////// 현재 프레임에 대한 정보를 Tracking 클래스 내에서 저장하기.///////////////
	if (mState == WORKING || mState == LOST)
		mCurrentFrame = Frame(im, cv_ptr->header.stamp.toSec(), mpORBextractor, mpORBVocabulary, mK, mDistCoef);  // ORB뽑은 프레임, mpORBextractor는 Tracking생성자에서 만들어진다
	else
		mCurrentFrame = Frame(im, cv_ptr->header.stamp.toSec(), mpIniORBextractor, mpORBVocabulary, mK, mDistCoef); // 다시 ORB 뽑음,           "        "
...
void Tracking::GrabImage(const sensor_msgs::ImageConstPtr& msg) // 이미지만 그랩하는게 아니라 tracking, relocalization, 등을 수행
{
...
void Tracking::FirstInitialization()
{
    ...
    if(mCurrentFrame.mvKeys.size()>100)
    {
        ...
        mLastFrame = Frame(mCurrentFrame);
        mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size()); // mCurrentFrame.mvKeysUn : 지금 들어온 한 프레임의 이미지에서 추출한 특징점들의 집합
